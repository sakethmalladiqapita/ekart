  ...
  return (
    <div style={{ maxWidth: '400px', margin: 'auto', padding: '32px' }}>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        ...
        <input type="text" name="street" placeholder="Street" value={form.address.street} onChange={handleChange} />
        <input type="text" name="city" placeholder="City" value={form.address.city} onChange={handleChange} />
        <input type="text" name="zip" placeholder="ZIP" value={form.address.zip} onChange={handleChange} />
        <button type="submit" style={{ padding: '10px', backgroundColor: '#2563eb', color: 'white', border: 'none', borderRadius: '8px' }}>
          Create User
        </button>
      </form>
      {message && <p style={{ marginTop: '20px', textAlign: 'center', color: '#4b5563' }}>{message}</p>}
    </div>
  );
};
